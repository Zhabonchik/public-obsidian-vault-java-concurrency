...
- `analytics`: what to use for analytics on your site. Values can be
  ...
  - `{ provider: 'plausible' }` (managed) or `{ provider: 'plausible', host: '<your-plausible-host>' }` (self-hosted, don't forget to add the `https://` in front of your provider domain!! Otherwise it will treat your domain as a subpath to the `baseUrl`, which does not work.): use [Plausible](https://plausible.io/);
