...
const ZONES = ["header", "left", "beforeBody", "afterBody", "right", "footer"] as const
...
function getZoneComponents(plugins: EnrichedPlugin[]): Record<Zone, ZoneComponent[]> {
...
export function LayoutPanel({ notify, onFocusChange }: LayoutPanelProps) {
  ...
  const zoneComponents = useMemo(() => {
    ...
    for (const zone of ZONES) {
      filtered[zone] = base[zone].filter((comp) => !override.exclude?.includes(comp.name))
      if (override.positions) {
        for (const [pluginName, newPosition] of Object.entries(override.positions)) {
          ...
          if (typeof newPosition === "string" && newPosition !== zone) {
            filtered[zone] = filtered[zone].filter((c) => c.name !== pluginName)
          }
        }
      }
  ...
  const getActiveZone = useCallback((): Zone => {
    ...
    return (["header", "beforeBody", "afterBody", "footer"] as const)[focusCenterRow]
    ...
    if (event.name === "k" && event.shift && selectedComp && selectedComponent > 0) {
      ...
      if (arrIdx >= 0 && aboveArrIdx >= 0) {
        ...
        if (myPlugin.layout && abovePlugin.layout) {
          updateLayout(arrIdx, { ...myPlugin.layout, priority: above.priority })
          updateLayout(aboveArrIdx, { ...abovePlugin.layout, priority: selectedComp.priority })
    ...
    if (
      ...
      if (arrIdx >= 0 && belowArrIdx >= 0) {
        ...
        if (myPlugin.layout && belowPlugin.layout) {
          updateLayout(arrIdx, { ...myPlugin.layout, priority: below.priority })
          updateLayout(belowArrIdx, { ...belowPlugin.layout, priority: selectedComp.priority })
          setSelectedComponent(selectedComponent + 1)
        ...
        onSelect={(newZone) => {
          ...
          if (arrIdx >= 0) {
            ...
            if (plugin.layout) {
              ...
              notify(`Moved ${selectedComp.displayName} to ${newZone}`, "success")
              setSelectedComponent(0)
            }
          }
          exitView()
  ...
  if (view === "edit-priority" && selectedComp) {
    return (
      <box flexDirection="column" padding={1}>
        ...
        <text>
          <span fg="#888888">Current: {selectedComp.priority} (lower = higher in zone)</span>
        </text>
        <box marginTop={1}>
          <text>Priority: </text>
  ...
  if (view === "edit-condition" && selectedComp) {
    return (
      <box flexDirection="column" padding={1}>
        ...
        <text>
          <span fg="#888888">
            Available: not-index, has-tags, has-backlinks, has-toc (or empty to remove)
          </span>
        </text>
        <box marginTop={1}>
          <text>Condition: </text>
              ...
              onSubmit={(value: string) => {
                ...
                if (arrIdx >= 0) {
                  ...
                  if (plugin.layout) {
                    ...
                    notify(
                      value.trim() ? `Condition set to ${value.trim()}` : "Condition removed",
                      "success",
                    )
                  }
                }
  ...
  if (view === "confirm-remove-layout" && selectedComp) {
    return (
      <box flexDirection="column" padding={1}>
        ...
        <text>
          <span fg="#888888">The plugin will remain installed but won't appear in the layout.</span>
        </text>
        <box marginTop={1}>
          ...
          <ConfirmPrompt
            onConfirm={() => {
              ...
              if (arrIdx >= 0) {
                ...
                notify(`Removed ${selectedComp.displayName} from layout`, "success")
              }
              exitView()
            }}
            onCancel={() => exitView()}
        ...
        {isDrilledIn ? (
          ...
          <box flexDirection="column">
            {visibleComps.map((comp) => (
              <text key={comp.name}>
                <span fg={isFocused ? "#AAAAAA" : "#666666"}>{` ${comp.displayName}`}</span>
              </text>
            ))}
            {overflowCount > 0 && (
              <text>
...
function MoveZoneView({ component, fromZone, zones, onSelect, onCancel }: MoveZoneViewProps) {
  useKeyboard((event) => {
...
function DisplayModeView({ component, fromZone, onSelect, onCancel }: DisplayModeViewProps) {
  useKeyboard((event) => {
    if (event.name === "escape") onCancel()
  })
...
function GroupsView({ layout, onSave, onCancel }: GroupsViewProps) {
  const groups = (layout as { groups?: Record<string, Record<string, unknown>> })?.groups ?? {}
              ...
              onSubmit={(value: string) => {
                if (value.trim()) {
                  ...
                  const currentGroups =
                    ...
                    >) ?? {}
                  currentGroups[value.trim()] = { direction: "row", gap: "0.5rem" }
  ...
  if (mode === "edit" && selectedGroup && editField) {
    ...
    const currentValue = String((groupConfig as Record<string, unknown>)[editField] ?? "")
          ...
          <box border borderStyle="single" height={3} flexGrow={1}>
            <input
              ...
              onSubmit={(value: string) => {
                ...
                currentGroups[groupName] = { ...currentGroups[groupName], [editField]: value }
                ;(newLayout as Record<string, unknown>).groups = currentGroups
...
type PageTypesMode = "list" | "add" | "confirm-delete" | "detail" | "exclude" | "positions"

function PageTypesView({ layout, plugins, notify, onSave, onCancel }: PageTypesViewProps) {
  const layoutConfig = (layout ?? {}) as LayoutConfig
  ...
  const updatePageType = useCallback(
    (pageType: string, updater: (current: PageTypeOverride) => PageTypeOverride) => {
      const newLayout = { ...(layout ?? {}) }
      const currentByPageType = {
        ...(((newLayout as LayoutConfig).byPageType as Record<string, PageTypeOverride>) ?? {}),
      }
      const current = { ...(currentByPageType[pageType] ?? {}) }
      const next = updater(current)

      if (next.exclude && next.exclude.length === 0) delete next.exclude
      if (next.positions && Object.keys(next.positions).length === 0) delete next.positions
    ...
    return (
      <PageTypeExcludeView
        ...
        onUpdate={(nextExclude) => {
          updatePageType(selectedPageType, (current) => ({ ...current, exclude: nextExclude }))
        }}
    ...
    return (
      <PageTypePositionsView
        ...
        onUpdate={(nextPositions) => {
          updatePageType(selectedPageType, (current) => ({ ...current, positions: nextPositions }))
        }}
        onCancel={() => setMode("detail")}
      />
    )
  ...
  const pageTypeOptions: SelectOption[] = pageTypeEntries.map(([name, override]) => {
    const cfg = override as PageTypeOverride
    const excludeCount = cfg.exclude?.length ?? 0
    const positionsCount = cfg.positions ? Object.keys(cfg.positions).length : 0
    return {
      name,
...
function PageTypePositionsView({
  ...
  const pluginOverrides = entries.filter((entry) => entry.kind === "plugin") as Array<
    Extract<PositionEntry, { kind: "plugin" }>
  >

  const existingOverrideNames = new Set(pluginOverrides.map((entry) => entry.pluginName))
