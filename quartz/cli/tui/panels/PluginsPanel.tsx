...
function ConfirmPrompt({ onConfirm, onCancel }: { onConfirm: () => void; onCancel: () => void }) {
  useKeyboard((event) => {
...
function getDefaultForKey(
  ...
  const defaults = manifest.defaultOptions as Record<string, unknown> | undefined
...
function normalizeCategories(category: string | string[] | undefined): string[] {
  ...
  if (Array.isArray(category)) return category.length > 0 ? category : ["unknown"]
  return [category]
}
...
export function PluginsPanel({ notify, onFocusChange, maxHeight }: PluginsPanelProps) {
  const { plugins, refresh, toggleEnabled, setPluginOrder, setPluginOptions } = usePlugins()
  ...
  const resolvePluginIndex = useCallback(
    (selectIdx: number): number | null => {
      ...
      if (val.type === "plugin" && typeof val.sortedIndex === "number") return val.sortedIndex
      return null
    },
    [listOptions],
  )
  ...
  useEffect(() => {
    if (editingKey !== null && selectedPlugin) {
      ...
      if (schema?.type === "enum" && schema.values && typeof currentValue === "string") {
  ...
  useKeyboard((event) => {
    ...
    if (event.name === "u") {
      ...
      import("../async-plugin-ops.js").then(({ tuiPluginUpdate }) => {
    ...
    if (event.name === "i") {
      ...
      import("../async-plugin-ops.js").then(({ tuiPluginInstall }) => {
    ...
    if (!editingKey && selectedPlugin) {
      ...
      if (editingSubKey && isEditableObject(currentValue)) {
        ...
        if (typeof currentSubValue === "boolean") {
          ...
          if (event.name === "return") {
            ...
            if (origIdx >= 0) {
              ...
              notify(`Set ${editingKey}.${editingSubKey} = ${newVal}`, "success")
            }
            setEditingSubKey(null)
          }
        }
        ...
        if (isEditableObject(currentValue)) {
          ...
          if (event.name === "up") {
            setHighlightedSubKeyIndex((prev) => (prev > 0 ? prev - 1 : totalItems - 1))
          }
          if (event.name === "down") {
            setHighlightedSubKeyIndex((prev) => (prev < totalItems - 1 ? prev + 1 : 0))
          }
          if (event.name === "return") {
          ...
          if (event.name === "x" && highlightedSubKeyIndex < subEntries.length) {
            const subKey = subEntries[highlightedSubKeyIndex]?.[0]
            if (subKey) {
              const origIdx = plugins.findIndex((p) => p.index === selectedPlugin.index)
              if (origIdx >= 0) {
                ...
                setHighlightedSubKeyIndex((prev) =>
                  prev >= subEntries.length - 1 ? Math.max(0, subEntries.length - 2) : prev,
                )
              }
            }
          }
          ...
          if (event.name === "up" && !event.shift) {
            setHighlightedArrayItemIndex((prev) => (prev > 0 ? prev - 1 : totalItems - 1))
          }
          if (event.name === "down" && !event.shift) {
            setHighlightedArrayItemIndex((prev) => (prev < totalItems - 1 ? prev + 1 : 0))
          }
          if (event.name === "return") {
          ...
          if (event.name === "x" && highlightedArrayItemIndex < currentValue.length) {
            const origIdx = plugins.findIndex((p) => p.index === selectedPlugin.index)
            if (origIdx >= 0) {
              const newArray = [...currentValue]
              newArray.splice(highlightedArrayItemIndex, 1)
              setPluginOptions(origIdx, editingKey, newArray)
              notify(`Removed ${editingKey}[${highlightedArrayItemIndex}]`, "success")
              setHighlightedArrayItemIndex((prev) =>
                prev >= newArray.length ? Math.max(0, newArray.length - 1) : prev,
              )
            }
          }
          if (
        ...
        if (
          ...
          if (event.name === "return") {
            ...
            if (origIdx >= 0 && newVal !== undefined) {
              ...
              setPluginOptions(origIdx, editingKey, newArray)
              notify(`Set ${editingKey}[${editingArrayIndex}] = ${newVal}`, "success")
            }
            setEditingArrayIndex(null)
          }
        }
  ...
  const optionSummary = useMemo(() => {
    if (!selectedPlugin?.options || typeof selectedPlugin.options !== "object") return "none"
    const keys = Object.keys(selectedPlugin.options)
    if (keys.length === 0) return "none"
    const defaults = (selectedPlugin.manifest as Record<string, unknown> | null)?.defaultOptions as
  ...
  if (loading) {
    ...
    return (
      <box flexDirection="column" padding={1} border borderStyle="single">
        ...
        <scrollbox stickyScroll stickyStart="bottom" focused marginTop={1} height={progressHeight}>
          ...
          <box border borderStyle="single" height={3} flexGrow={1}>
            <input
              ...
              onSubmit={(value: string) => {
                ...
                import("../async-plugin-ops.js").then(({ tuiPluginAdd }) => {
                  tuiPluginAdd([source], (msg: string) => {
                    ...
                    .then((result) => {
                      refresh()
                      if (result.success) {
                        notify(`Added ${result.installed ?? 0} plugin(s)`, "success")
                      } else {
        ...
        <box marginTop={1}>
          ...
          <ConfirmPrompt
            onConfirm={() => {
              ...
              exitView()
              setLoading(true)
              setProgressMessages([`→ Removing ${selectedPlugin.displayName}...`])
              import("../async-plugin-ops.js").then(({ tuiPluginRemove }) => {
                tuiPluginRemove([pluginName], (msg: string) => {
                  ...
                  .then((result) => {
                    ...
                    if (result.success) {
                      notify(`Removed ${selectedPlugin.displayName}`, "success")
                    } else {
  ...
  const renderOptionsPanel = () => {
    ...
    const renderEditPanel = () => {
      if (!editingKey) {
        ...
        return (
          <box flexDirection="column" border borderStyle="single" paddingX={1}>
            ...
            <box marginTop={1}>
              <text>
                <span fg="#888888">Enter: edit │ D: restore default │ Esc: close options</span>
              </text>
            </box>
          </box>
        )
          ...
          return (
            <box flexDirection="column" border borderStyle="single" paddingX={1}>
              ...
              {defaultSubValue !== undefined && !isDefaultSub && (
                <text>
                  <span fg="#555555">Default: {formatValue(defaultSubValue)}</span>
                </text>
              )}
      ...
      if (editingKey && editingArrayIndex !== null && Array.isArray(currentValue)) {
        const currentItem = currentValue[editingArrayIndex]
        const defaultArr = Array.isArray(defaultVal) ? defaultVal : undefined
        const defaultItem = defaultArr ? defaultArr[editingArrayIndex] : undefined
                ...
                <input
                  ...
                  onSubmit={(value: string) => {
                    ...
                    if (origIdx >= 0) {
                      ...
                      setPluginOptions(origIdx, editingKey, newArray)
                      notify(`Set ${editingKey}[${editingArrayIndex}]`, "success")
                    }
                    setEditingArrayIndex(null)
                  }}
                />
      ...
      if (isEditableObject(currentValue)) {
                  ...
                  <input
                    ...
                    onSubmit={(value: string) => {
                      ...
                      if (origIdx >= 0) {
                        ...
                        setPluginOptions(origIdx, editingKey, updatedParent)
                        notify(`Added ${editingKey}.${newObjectKeyName}`, "success")
                      }
                      setNewObjectKeyName(null)
                      setAddingObjectKey(false)
                    }}
        ...
        return (
          <box flexDirection="column" border borderStyle="single" paddingX={1}>
            ...
            <scrollbox>
              <box flexDirection="column">
                {subEntries.map(([subKey, subVal], i) => {
                  const subDefault = isEditableObject(defaultVal) ? defaultVal[subKey] : undefined
                ...
                <text key="__add__">
                  <span fg={highlightedSubKeyIndex === subEntries.length ? "#FFFFFF" : "#888888"}>
            ...
            <text>
              <span fg="#888888">↑↓: navigate │ Enter: edit field │ x: delete │ Esc: back</span>
            </text>
          </box>
        )
      }
          ...
          <box flexDirection="column" border borderStyle="single" paddingX={1}>
            ...
            <scrollbox>
              <box flexDirection="column">
                ...
                <text key="__add__">
                  <span
                    fg={highlightedArrayItemIndex === currentValue.length ? "#FFFFFF" : "#888888"}
                  >
                    {highlightedArrayItemIndex === currentValue.length ? "▸ " : "  "}
                    {addItemLabel}
                  </span>
        ...
        <scrollbox>
          <box flexDirection="column">
            {optionEntries.map(([key, value], i) => {
              ...
              const typeTag = isEditableObject(value) ? " {…}" : Array.isArray(value) ? " […]" : ""
              const isActive = editingKey === key
              const isHighlighted = !editingKey && i === highlightedOptionIndex
              const fg = isActive ? "#FFFF00" : isHighlighted ? "#FFFFFF" : "#888888"
              ...
              <box marginTop={1} flexDirection="column">
                ...
                {selectedPlugin.currentCommit && (
                  <text>
                    ...
                    {selectedPlugin.modified && <span fg="yellow"> (modified)</span>}
                  </text>
                )}
                ...
                {optionSummary !== "none" && (
                  <text>
                    <span fg="#555555">{"           * = modified from default"}</span>
                  </text>
                )}
              </box>
            </box>
